...
Weekly pages are published on Wednesdays. The build releases each week's content at 13:00 Europe/Zurich and the site displays "Available after <lecture date>, 13:00". To preview the site at a specific release point, set `BIGHT_BUILD_TIME` to an ISO timestamp:

```bash
BIGHT_BUILD_TIME=2026-10-28T12:01:00+00:00 python build_site.py
```
...
The `.github/workflows/pages.yml` workflow builds and deploys the site on pushes to `main`. A scheduled run fires every Wednesday at 13:00 Europe/Zurich to unlock the week. GitHub Actions `schedule` delivery can occasionally be delayed or skipped, so there is also an easy manual trigger.
...
The rebuild uses the current time, so as soon as a week's content is due (any time after the 13:00 Europe/Zurich release moment) it unlocks immediately. You can also trigger it from the GitHub UI: **Actions → Deploy static site to GitHub Pages → Run workflow**.
